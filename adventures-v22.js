...
function cleanupTransientUi(){
  ...
  qAll('.va-processing,.va-scene-transition,.va-banner-stack,.va-dialogue-overlay,.va-dice-overlay').forEach(x=>x.remove());
...
function drawSceneLife(ctx,time,scene,daylight,dusk){
  ...
  async function travel(id){const dest=safeArray(state?.map_locations).find(x=>x.id===id);const prev=snapshotState();showSceneTransition('TRAVEL', dest?.real_name||dest?.name||'On the road');showProcessing('TRAVEL', `Heading to ${dest?.real_name||dest?.name||'the next stop'}`, 'Packing satchel, checking weather and taking the road.');const res=await callRpc('adventure_phase2_travel',{p_location_id:id});takePhase2Result(res);view='game';lastRenderedNarrative='';lastNarrative=state.adventure.last_summary;render();renderStateFeedback(prev,state,'travel');hideSceneTransition();saved()}
