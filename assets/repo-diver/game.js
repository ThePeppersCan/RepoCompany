(() => {
  ...
  function campaignRadio(speaker,text,duration=4200){const el=$('rdCampaignRadio');if(!el)return;el.classList.remove('hidden');el.innerHTML=`<small>${rdEsc(String(speaker||'TIDELINE').toUpperCase())}</small><b>${rdEsc(text)}</b>`;clearTimeout(campaignRadioTimer);campaignRadioTimer=setTimeout(()=>el.classList.add('hidden'),duration);try{A.play?.('radio')}catch(_){}}
  ...
  function renderResearchView(){
    ...
    el.innerHTML=`<div class="rd-research-summary"><article><small>CAMERA ARCHIVE</small><b>${photos}</b><span>field photographs</span></article><article><small>OBSERVED SPECIES</small><b>${observed}</b><span>documented subjects</span></article><article><small>ECOLOGY NOTES</small><b>${ecoObs.length}</b><span>unique behaviours documented</span></article><article><small>ACTIVE TAGS</small><b>${tags.length}</b><span>research animals that may return</span></article></div>
      ...
      <div class="rd-tag-ledger"><div><small>RESEARCH TAG LEDGER</small><h4>${tags.length?`${tags.length} ACTIVE FIELD TAG${tags.length===1?'':'S'}`:'NO ACTIVE TAGS'}</h4><p>Craft the Research Dart Array, aim at wildlife and press <b>R</b>. Tagged animals can unexpectedly return on later dives in the same region.</p></div>${tags.slice(0,12).map(t=>`<article><b>${rdEsc(t.tag_code||'MR-TAG')}</b><span>${rdEsc(D.fishById(t.fish_id)?.name||t.fish_id)}</span><small>${rdEsc(D.biome(t.biome)?.short||t.biome)} · ${Number(t.sightings||1)} SIGHTING${Number(t.sightings||1)===1?'':'S'}</small></article>`).join('')}</div>
  ...
  function hud(){
    if(!run)return;const co=campaignObjectiveMarkup(),objective=$('rdCampaignObjective');if(objective){objective.classList.toggle('hidden',!co);objective.classList.toggle('assist',!!settings.storyAssist);if(co){objective.querySelector('small').textContent=co.act;objective.querySelector('b').textContent=co.title;objective.querySelector('span').textContent=co.stage+(settings.storyAssist&&run.campaign?.location_id?` · ${D.locationById?.(run.campaign.location_id)?.name||'FOLLOW SITE BEACON'}`:'');objective.querySelector('em').textContent=co.progress;}}$('rdO2Fill').style.width=run.player.o2+'%';$('rdHpFill').style.width=run.player.hp+'%';const band=E.depthBand(run),depth=Math.round((run.player.y/540)*run.biome.max_depth);$('rdDepth').textContent=depth+'m';$('rdDepthBand')&&($('rdDepthBand').textContent=run.interior?(run.interior.site.rooms[Math.min(run.interior.room,run.interior.site.rooms.length-1)]?.name||'INTERIOR'):band.label);$('rdBiomeName')&&($('rdBiomeName').textContent=run.interior?run.interior.site.name:(run.campaign?.type==='finale'||run.campaign?.type==='depth'?'UNCHARTED DEPTHS':run.biome.short));
  ...
  function renderRestaurantMenu() {
    ...
    $('rdRestaurantMenu').innerHTML = selectedRecipes.map(id => {
      ...
      return `<button data-cook="${id}" data-category="${category}" ${disabled ? 'disabled' : ''}><i class="rd-dish-glyph ${category}"></i><span class="rd-menu-copy"><small>${category.toUpperCase()} · ${r?.base_price?.toLocaleString?.() || 0} GP</small><b>${r?.name || id}</b><span>TONIGHT'S STOCK <strong>${stock}</strong></span><u><i style="width:${Math.min(100,stock/stockMax*100)}%"></i></u></span><em>${disabled && stock <= 0 ? 'SOLD OUT' : service.cook ? 'LINE BUSY' : 'COOK'}</em></button>`;
    }).join('');
  ...
  addEventListener('keydown', e => {
    ...
    if (key === 'q' && diving) {E.useSonar(run, profile.equipment);markTutorial('sonar');}
