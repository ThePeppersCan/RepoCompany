...
-- SAFE RE-RUN VERSION: drops only RPC function definitions, never binder data.
...
-- If an earlier/incomplete version of the table already exists, make sure the
-- newer columns are present before recreating the RPCs.
alter table public.quidditch_tcg_binder_layouts
  add column if not exists username text,
  add column if not exists layout jsonb default '[]'::jsonb,
  add column if not exists storage jsonb default '[]'::jsonb,
...
-- has changed. Drop only the RPC definitions first; this preserves every row in
-- quidditch_tcg_binder_layouts.
drop function if exists public.set_my_quidditch_tcg_binder_layout(text,jsonb,jsonb);
drop function if exists public.get_my_quidditch_tcg_binder_layout();
drop function if exists public.get_public_quidditch_tcg_binder_layout(text);

create function public.set_my_quidditch_tcg_binder_layout(
  p_username text,
  p_layout jsonb,
...
create function public.get_my_quidditch_tcg_binder_layout()
returns table(username text, layout jsonb, storage jsonb, updated_at timestamptz)
language sql
security definer
set search_path = public, auth
...
create function public.get_public_quidditch_tcg_binder_layout(p_username text)
returns table(username text, layout jsonb, storage jsonb, updated_at timestamptz)
language sql
security definer
