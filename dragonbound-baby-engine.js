/* Velmora Dragonbound — autonomous baby dragon engine V34.00 Furniture Use States & Durations */
(()=>{
  'use strict';
...
const HOUSE_MAPS={
  ...
  const BOND_STAGES=[
    ...
    {min:60,name:'Attached',note:'Feels secure around you'},
    {min:80,name:'Best Friend',note:'Recognises treat time almost instantly'},
  ...
  const stage=this.bondStage();const personalityLabel=this.personalityReaction?.('pet')||'',petLabel=personalityLabel||(this.bond>=95?'dragonbound ♥':this.bond>=80?'best friends!':this.bond>=60?'loves the scratches!':this.bond>=40?'nuzzles closer!':this.bond>=20?'happy!':'lovely!');this.playPetLove(petLabel);this.applyCareBenefit('social',14);this.applyCareBenefit('comfort',8);this.applyCareBenefit('fun',3);this.awardBond('pet',1.0,115000);const obs=this.memory.observationCounters||(this.memory.observationCounters={});obs.petsReceived=(Number(obs.petsReceived)||0)+1;this.noteUniverseActivity?.('keeper:pet');this.maybeShowDragonThought?.('pet');try{window.dispatchEvent(new CustomEvent('dragonbound:keeper-pet',{detail:{dragonId:String(this.dragon?.id||''),dragonName:String(this.dragon?.name||'Your dragon')}}));}catch(_e){}this.rememberLifeEvent('bond','First proper cuddle','You gave your dragon a reassuring pet.','first-pet');this.engine.saveBehaviourLocal?.();this.updatePetHint();return true;
    ...
    maybeShowDragonThought(event='',ctx={}){
      ...
      if(e==='pet'){
        ...
        const meta=ctx.meta||{},kind=String(ctx.kind||''),isNew=!!meta.isNew,isFavourite=this.thoughtFavourite(meta,kind),name=String(meta.name||'this').slice(0,38);
        type=isNew?'new-furniture':isFavourite?'favourite':'activity';priority=isNew?DRAGON_THOUGHT_PRIORITY.newFurniture:isFavourite?34:DRAGON_THOUGHT_PRIORITY.activity;chance=isNew?.82:isFavourite?.48:.26;cooldownMs=isNew?180000:42000;cooldownKey=isNew?`new-furniture:${meta.placementId||meta.itemId}`:`furniture:${kind}`;clearOnStateChange=!isNew&&!isFavourite;
        if(!isNew&&!isFavourite&&nowMs<this.nextActivityThoughtAt)return false;
        if(isNew){
    ...
    scheduleReturnMoment(){
      clearTimeout(this.returnMomentTimer);this.returnMomentTimer=0;if(this.awayMs<3*60*60*1000)return;this.returnMomentTimer=setTimeout(()=>{if(!this.el?.isConnected)return;const c=this.careStats(),mood=this.moodSummary(),life=this.lifeData();let text='Noticed you came home';if(c.hunger<35)text='Immediately checking where food might be';else if(c.energy<32)text='Sleepily noticed you came home';else if(mood==='Grumpy'||mood==='Sulking')text='A little grumpy, but keeping an eye on you';else if(this.hasTrait('Greeting Enthusiast')||this.hasTrait('Cuddlebug')||this.hasTrait('Shadow'))text='Could not be more obvious about being happy you are home';else if(this.hasTrait('Independent Friend'))text='Looks up, checks that it is you, then calmly carries on';else if(this.hasTrait('Shy'))text=this.bond>=60?'Quietly comes closer once it realises you are home':'Keeps a cautious eye on you from somewhere familiar';else if(this.hasTrait('Lazy'))text='Acknowledges your return without making the mistake of standing up';else if(this.bond>=60)text='Very happy to see you again';this.lifeEventNotice(text,this.bond>=60?'happy':'');life.lastReturnAt=Date.now();life.lastMeaningfulMoment={type:'return-home',label:'Keeper came home',detail:text,at:Date.now()};this.noteUniverseActivity('keeper:greeting');this.behaviourDirty=true;},1800);
    }
...
bondRelationship(){
  const current=this.bondStage(),next=this.nextBondStage(),fav=this.preferences?.formed?.favouriteFurniture?.name||'';
  return {level:Math.max(1,Math.round(this.bond)),name:current.name,note:current.note,next,nextAt:next?.min||100,favouriteFurniture:fav};
}
addBond(amount=0){
  ...
  if(Math.abs(this.bond-before)>=.01){
    ...
    if(afterStage.min>beforeStage.min&&!this.memory.bondMilestones?.[String(afterStage.min)]){
      ...
      this.playPetLove(afterStage.name==='Dragonbound'?'dragonbound ♥':'trusts you more!');
    }
  }
  return this.bond;
}
...
scheduleBondGreeting(){
  clearTimeout(this.greetingTimer);this.greetingTimer=0;const eager=this.hasTrait('Clingy')||this.hasTrait('Greeting Enthusiast')||this.hasTrait('Shadow')||this.hasTrait('Cuddlebug')||this.hasTrait('Affectionate')||this.hasTrait('Keeper-Focused'),reserved=this.hasTrait('Shy')||this.hasTrait('Independent')||this.hasTrait('Independent Friend')||this.hasTrait('Suspicious of Strangers');const minimum=eager?25:reserved?55:40;if(this.bond<minimum)return;
  const last=Number(this.memory.lastGreetingAt||0),cooldown=eager?14*60*1000:reserved?28*60*1000:20*60*1000;if(last&&Date.now()-last<cooldown)return;if(reserved&&Math.random()>.58)return;
  this.greetingTimer=setTimeout(()=>this.startBondGreeting(),eager?720:reserved?1850:1200);
}
startBondGreeting(){
  const eager=this.hasTrait('Clingy')||this.hasTrait('Greeting Enthusiast')||this.hasTrait('Shadow')||this.hasTrait('Cuddlebug')||this.hasTrait('Affectionate')||this.hasTrait('Keeper-Focused'),reserved=this.hasTrait('Shy')||this.hasTrait('Independent')||this.hasTrait('Independent Friend')||this.hasTrait('Suspicious of Strangers'),minimum=eager?25:reserved?55:40;
  ...
  const floor=this.map.floors.find(f=>f.id===this.floorId),nodes=(floor?.navigationNodes||[]).filter(p=>this.engine.isWalkable(this.floorId,p));
  const target=nodes.slice().sort((a,b)=>((Math.abs(a[0]-.5)*220-a[1]*120)-(Math.abs(b[0]-.5)*220-b[1]*120)))[0];
  if(!target)return false;this.memory.lastGreetingAt=Date.now();this.noteUniverseActivity('keeper:greeting');try{window.dispatchEvent(new CustomEvent('dragonbound:keeper-greeting',{detail:{dragonId:String(this.dragon?.id||''),dragonName:String(this.dragon?.name||'Your dragon')}}));}catch(_e){}this.behaviourDirty=true;this.path=[];this.pathIndex=0;this.stateUntil=0;this.pauseUntil=0;
  const started=this.startWalk(target.slice(),'bond-greeting');if(started){this.walkSpeedBoost=this.hasTrait('Greeting Enthusiast')||this.hasTrait('Hyper')?1.42:this.bond>=80?1.35:this.hasTrait('Lazy')?1.08:1.18;this.personalityExpression(this.hasTrait('Shy')?'…':'♥',this.hasTrait('Shy')?'hello':'you came back','greeting');}return started;
}
...
stopBathAudio(){
  ...
  try{audio.pause();audio.currentTime=0;}catch(_){ }
    ...
    beginFurnitureUse(){
      ...
      if(plan.commanded&&!this.currentLifeEvent&&['sleep','rest','play','puzzle','eat','drink','wash','sandbath','groom','exercise','scratch','warm'].includes(kind))this.awardBond(`guided-${kind}`,.28,8*60*1000);
    ...
    playPendingRaceReaction(){
      ...
      this.noteUniverseActivity(won?'race:win':'race:finish');this.behaviourDirty=true;this.engine.saveBehaviourLocal?.();this.engine.saveBehaviour?.(true);this.nextDecision=now()+3800;return true;
    }
  ...
  class DragonboundBabyEngine{
    ...
    updateNeedsHud(force=false){
      ...
      const care=actor.careStats(),name=actor.dragon?.name||actor.def?.displayName||'Baby Dragon',mood=actor.moodSummary();
