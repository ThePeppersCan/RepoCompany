...
const ARCHETYPE_ICONS={Warrior:'⚔',Ranger:'🏹',Rogue:'🗡',Mage:'✧',Alchemist:'⚗',Bard:'♫',Guardian:'🛡',Beastkeeper:'🐾',Investigator:'🔎'};
  ...
  let state=null,phase2={},phase3={},phase4={},phase5={},phase5Shops=[],view='entry',drawer=null,busy=false,sceneFrame=0,heartbeat=0,saveToastTimer=0,lastNarrative='',activeConversation=null,lastNpcKey=null,narrativeTimer=0,lastRenderedNarrative='',uiFxQueue=[],fxActive=false,processingState=null,transitionTimer=0,placeholderTimer=0,placeholderIndex=0,dynamicSuggestions=[],commandHistory=[],commandIndex=-1,pendingAmbiguityRaw='',lastPhase3Debug=null,mapRegionKey=null;
...
async function loadFullState(opts={}){
  ...
  if(!base?.exists){phase2={};phase3={};phase4={};phase5={};phase5Shops=[];return base}
...
function takePhase2Result(res){
  if(res?.base){
    ...
    state=mergePhase5(mergePhase4(mergePhase3(mergePhase23(mergePhase2(res.base,res.phase2||{}),res.phase23||existing23),res.phase3||existing3),res.phase4||existing4),res.phase5||existing5);return res;
  }
  return res;
}
...
function renderEncounterStrip(){
  ...
  function renderActions(){
    ...
    if(safeArray(phase5Shops).length)actions.push(['open-shop','Visit local shops']);
...
function drawHighweald(ctx,s,time,scene){
  ...
  function renderDrawer(){const d=el('vaDrawer');if(!d)return;if(!drawer){d.className='va-drawer';d.innerHTML='';return}d.className='va-drawer open';let title='',body='';
    ...
    if(drawer==='shops'){title='Shops & Supplies';body=renderShops()}
