...
begin
  ...
  -- A browser can finish/die immediately after completing a rooftop. Older
  -- clients marked the run as finalising before their last queued level write,
  -- which could leave either the validated level count OR only the validated
  -- height a little behind the real run. Reconcile only a believable tail:
  -- server time, minimum height-per-level, maximum height-per-level, run speed,
  -- and a small missing-level window must all agree.
  if coalesce(p_rooftop_level,0)>v_recorded_levels
     or v_height > greatest(120,v_recorded_height+120) then
    if coalesce(p_rooftop_level,0) <= greatest(0,floor(v_server_ms/2500.0)::integer)
       ...
       and coalesce(p_rooftop_level,0) <= v_recorded_levels+16
       and v_height <= greatest(500,coalesce(p_rooftop_level,0)*140+600)
       and v_height <= 250+floor((v_server_ms/1000.0)*130)::integer then
      v_levels := greatest(v_recorded_levels,greatest(0,coalesce(p_rooftop_level,0)));
      ...
      update public.repo_rooftops_runs r
         set completed_levels=greatest(r.completed_levels,v_levels),
             highest_height=greatest(r.highest_height,v_recorded_height)
       where r.id=p_run_id;
    else
      if coalesce(p_rooftop_level,0)>v_recorded_levels then
        raise exception 'One or more rooftop levels were not validated';
      else
        raise exception 'Final height exceeds the validated route';
      end if;
    end if;
  end if;
