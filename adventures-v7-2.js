...
const ARCHETYPE_ICONS={Warrior:'⚔',Ranger:'🏹',Rogue:'🗡',Mage:'✧',Alchemist:'⚗',Bard:'♫',Guardian:'🛡',Beastkeeper:'🐾',Investigator:'🔎'};
  ...
  const notify=(m)=>{
    const message=String(m||'Something went wrong.');
    try{
      const host=q(`#${DIALOG_ID} .va-shell`);
      if(!host){console.info('[Adventures]',message);return}
      let stack=host.querySelector('.va-notice-stack');
      if(!stack){stack=document.createElement('div');stack.className='va-notice-stack';host.appendChild(stack)}
      const note=document.createElement('div');note.className='va-notice';note.setAttribute('role','status');note.textContent=message;stack.appendChild(note);
      requestAnimationFrame(()=>note.classList.add('show'));
      setTimeout(()=>{note.classList.remove('show');setTimeout(()=>{note.remove();if(!stack.children.length)stack.remove()},220)},3200);
    }catch(_){console.info('[Adventures]',message)}
  };
...
function renderCreate(){
  const stage=el('vaStage');stage.className='va-stage va-stage-create';
...
function renderGame(){
  ...
  const stage=el('vaStage');stage.className='va-stage va-stage-game';
  ...
  renderDrawer();startSceneLoop();afterGameRender();bindNearbyPeople();
}


function bindNearbyPeople(){
  qAll(`#${DIALOG_ID} .va-nearby-person[data-npc]`).forEach(btn=>{
    if(btn.dataset.vaTalkBound==='1')return;
    btn.dataset.vaTalkBound='1';
    btn.addEventListener('click',e=>{
      e.preventDefault();e.stopPropagation();
      if(btn.classList.contains('opening'))return;
      btn.classList.add('opening');
      playClick();
      talkNpc(btn.dataset.npc,btn.dataset.topic||'hello').catch(err=>{
        btn.classList.remove('opening');
        hideProcessing();
        console.error('[Velmora Adventures]',err);
        notify(err?.message||'That conversation could not be opened.');
      });
    });
  });
}
...
async function acceptQuest(key){const prev=snapshotState();showProcessing('MAIN STORY','Accepting quest','A simple errand has a habit of becoming something else.');const base=await callRpc('adventure_accept_quest',{p_quest_key:key});const p2=await callRpc('adventure_phase2_get_state',{}, {silent:true});state=mergePhase2(base,p2);await refreshPhase3();lastRenderedNarrative='';lastNarrative='A courier presses a green-corded parcel into your hands. “Willowmere. Straight there, ideally.”';render();renderStateFeedback(prev,state,'quest');saved()}
  ...
  async function talkNpc(key,topic='hello'){
    ...
    try{await refreshPhase3()}catch(err){console.warn('[Velmora Adventures] Phase 3 refresh skipped during dialogue',err)}
    try{await refreshPhase7()}catch(err){console.warn('[Velmora Adventures] Phase 7 refresh skipped during dialogue',err)}
