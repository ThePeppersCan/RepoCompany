-- Repo Company V22.11 — PSYCOMPANY Promos
-- Deploy the matching assets/script before running this migration.
--
-- Normal pack rarity ladder after this patch:
-- Black Label 1% | Millennium 2% | Signature 3% | Legendary 4% | Rival 6% | Platinum 7%
-- PSYCOMPANY Promos 12% | Patch 14% | Velmora: Off the Broom 16% | Full Art 19% | Standard 16%
-- PSYCOMPANY Promos sit just above Patch in the rarity ladder, as requested.

insert into public.repo_tcg_pack_cards(card_id,rarity,duplicate_protected)
values
  ('psycompany_promo_besquelcher','promo',false),
  ('psycompany_promo_debbie','promo',false),
  ('psycompany_promo_nimbler_2000','promo',false),
  ('psycompany_promo_pipsqueak','promo',false),
  ('psycompany_promo_rocky','promo',false),
  ('psycompany_promo_soup','promo',false)
...
declare
  ...
  v_standard text[]; v_full_art text[]; v_off_broom text[]; v_patch text[]; v_promo text[]; v_platinum text[]; v_rival text[]; v_legendary text[]; v_signature text[]; v_millennium text[]; v_black_label text[];
...
begin
  ...
  elsif cardinality(v_rival)>0 and v_roll<0.16 then v_card:=v_rival[1+floor(random()*cardinality(v_rival))::integer];
  elsif cardinality(v_platinum)>0 and v_roll<0.23 then v_card:=v_platinum[1+floor(random()*cardinality(v_platinum))::integer];
  elsif cardinality(v_promo)>0 and v_roll<0.35 then v_card:=v_promo[1+floor(random()*cardinality(v_promo))::integer];
  elsif cardinality(v_patch)>0 and v_roll<0.49 then v_card:=v_patch[1+floor(random()*cardinality(v_patch))::integer];
  elsif cardinality(v_off_broom)>0 and v_roll<0.65 then v_card:=v_off_broom[1+floor(random()*cardinality(v_off_broom))::integer];
  elsif cardinality(v_full_art)>0 and v_roll<0.84 then v_card:=v_full_art[1+floor(random()*cardinality(v_full_art))::integer];
