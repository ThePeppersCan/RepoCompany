-- Repo Company V22.09 — KABOOM Platinum + Velmora: Off the Broom
-- Deploy the matching assets/script before running this migration.
--
-- Normal pack rarity ladder after this patch:
-- Millennium 2% | Signature 3% | Legendary 4% | Rival 7% | Platinum 8%
-- Patch 16% | Velmora: Off the Broom 18% | Full Art 22% | Standard 20%
-- Off the Broom therefore sits directly above Full Art, as requested.

insert into public.repo_tcg_pack_cards(card_id,rarity,duplicate_protected)
values
  ('besquelcher_black_label','black_label',false),
  ('debbie_black_label','black_label',false),
  ('nimbler_2000_black_label','black_label',false),
  ('pipsqueak_black_label','black_label',false),
  ('rocky_black_label','black_label',false),
  ('soup_black_label','black_label',false),
  ('besquelcher_kaboom_platinum','platinum',false),
  ('debbie_kaboom_platinum','platinum',false),
  ('dopey_dom_kaboom_platinum','platinum',false),
  ('jenny_kaboom_platinum','platinum',false),
  ('jud_kaboom_platinum','platinum',false),
  ('mad_rager_kaboom_platinum','platinum',false),
  ('nimbler_2000_kaboom_platinum','platinum',false),
  ('pipsqueak_kaboom_platinum','platinum',false),
  ('rocky_kaboom_platinum','platinum',false),
  ('soup_kaboom_platinum','platinum',false),
  ('off_the_broom_besquelcher_hair_day','off_the_broom',false),
  ('off_the_broom_debbie_petals_before_practice','off_the_broom',false),
  ('off_the_broom_dopey_dom_breakfast_disaster','off_the_broom',false),
  ('off_the_broom_jenny_a_quiet_morning','off_the_broom',false),
  ('off_the_broom_jud_gentle_giant','off_the_broom',false),
  ('off_the_broom_mad_rager_just_add_water','off_the_broom',false),
  ('off_the_broom_nimbler_2000_routine_maintenance','off_the_broom',false),
  ('off_the_broom_pipsqueak_first_frost','off_the_broom',false),
  ('off_the_broom_rocky_market_regular','off_the_broom',false),
  ('off_the_broom_soup_a_little_garden','off_the_broom',false)
...
declare
  ...
  v_standard text[]; v_full_art text[]; v_off_broom text[]; v_patch text[]; v_platinum text[]; v_rival text[]; v_legendary text[]; v_signature text[]; v_millennium text[]; v_black_label text[];
...
begin
  ...
  -- Standard remains duplicate-protected; every special rarity can produce another physical copy.
  ...
  elsif cardinality(v_rival)>0 and v_roll<0.17 then v_card:=v_rival[1+floor(random()*cardinality(v_rival))::integer];
  elsif cardinality(v_platinum)>0 and v_roll<0.25 then v_card:=v_platinum[1+floor(random()*cardinality(v_platinum))::integer];
  elsif cardinality(v_patch)>0 and v_roll<0.41 then v_card:=v_patch[1+floor(random()*cardinality(v_patch))::integer];
  elsif cardinality(v_off_broom)>0 and v_roll<0.59 then v_card:=v_off_broom[1+floor(random()*cardinality(v_off_broom))::integer];
  elsif cardinality(v_full_art)>0 and v_roll<0.81 then v_card:=v_full_art[1+floor(random()*cardinality(v_full_art))::integer];
