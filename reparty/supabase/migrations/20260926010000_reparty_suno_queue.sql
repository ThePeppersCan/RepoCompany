-- Apply after the Game mode migration. Additive; keeps all rooms, playlists and games.
-- Suno uses the official embed and a duration-based queue timer, not synchronized media.
begin;
...
begin
  ...
  -- Timer requests are checked against server time AND the exact playback run.
  -- This prevents early advances and duplicate skips on single-song repeat.
  if p_action='suno_next' then
    if coalesce((p_data->>'revision')::bigint,-1)<>r.revision
      or p_data->>'item_id' is distinct from pb->>'item_id'
      or p_data->>'started_at' is distinct from pb->>'updated_at' then
      return jsonb_build_object('stale',true);
    end if;
    select track into entry from jsonb_array_elements(lists) list,
      lateral jsonb_array_elements(list->'items') track where track->>'id'=pb->>'item_id';
    if pb->>'video_id' not like 'suno:%' or not coalesce((pb->>'playing')::boolean,false)
      or entry->>'duration' is null
      or extract(epoch from stamp-(pb->>'updated_at')::timestamptz) + coalesce((pb->>'position')::double precision,0)
        < (entry->>'duration')::double precision + 5 then
      p_action := 'snapshot';
    else p_action := 'next';
    end if;
  end if;
  if p_action in ('join','snapshot') then null;
    ...
    if p_action='playlist_shuffle' then
      ...
      if coalesce(p_data->>'video_id','') !~ '^([a-zA-Z0-9_-]{11}|suno:[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})$' then
        raise exception 'Use a valid YouTube or Suno song link.';
      end if;
      ...
      if p_data->>'video_id' like 'suno:%' then
        if jsonb_typeof(p_data->'duration') is distinct from 'number' then raise exception 'Suno needs a song duration.'; end if;
        pos := (p_data->>'duration')::double precision;
        if pos is null or not(pos>=1 and pos<=86400) then raise exception 'Invalid Suno duration.'; end if;
        entry := entry || jsonb_build_object('duration',pos);
      end if;
    ...
    -- Suno cannot seek through its embed. Shared Play is always a fresh start.
    if pb->>'video_id' like 'suno:%' and (p_data->>'playing')::boolean then pos := 0; end if;
  ...
  select jsonb_build_object(
    ...
    'room',(to_jsonb(r)-'settings') || jsonb_build_object('settings',coalesce(r.settings,'{}'::jsonb)-'played','suno_queue',true),
