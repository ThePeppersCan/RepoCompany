...
returns table(card_id text, owned_cards text[], bank_items jsonb, skill_one text, skill_one_xp integer, skill_one_total integer, skill_two text, skill_two_xp integer, skill_two_total integer, all_cards_owned boolean)
language plpgsql
security definer
...
declare
  ...
  v_skill_one text; v_skill_two text; v_column_one text; v_column_two text;
  v_total_one integer; v_total_two integer; v_username text;
  v_standard_available text[]; v_full_art_available text[]; v_patch_available text[]; v_platinum_available text[]; v_rival_available text[]; v_legendary_available text[]; v_signature_available text[]; v_millennium_available text[];
...
begin
  ...
  if v_username='admin' then
    v_quantity:=greatest(1,v_quantity); v_items:=jsonb_set(v_items,'{quidditch_tcg_pack}','1'::jsonb,true);
  ...
  select coalesce(array_agg(card order by ord),'{}'::text[]) into v_available
  from unnest(v_all_cards) with ordinality as available(card,ord) where not (card=any(v_owned));
  if cardinality(v_available)=0 then raise exception 'You already own every current Quidditch TCG pack card. The pack was not consumed.'; end if;

  -- Duplicate-proof rarity pools. Patch is deliberately a little rarer than Full Art.
  -- Millennium 2%, Signature 3%, Gold Legendary 4%, Rival 7%, Platinum 8%, Patch 16%, Full Art 22%, Standard 38%.
  select coalesce(array_agg(card),'{}'::text[]) into v_millennium_available from unnest(v_available) card where card like '%_millennium';
  select coalesce(array_agg(card),'{}'::text[]) into v_signature_available from unnest(v_available) card where card like '%_signature';
  select coalesce(array_agg(card),'{}'::text[]) into v_legendary_available from unnest(v_available) card where card like '%_legendary_full_art';
  select coalesce(array_agg(card),'{}'::text[]) into v_rival_available from unnest(v_available) card where card like '%_rival';
  select coalesce(array_agg(card),'{}'::text[]) into v_platinum_available from unnest(v_available) card where card like '%_platinum';
  select coalesce(array_agg(card),'{}'::text[]) into v_patch_available from unnest(v_available) card where card like '%_patch';
  select coalesce(array_agg(card),'{}'::text[]) into v_full_art_available from unnest(v_available) card
    where card like '%_full_art' and card not like '%_legendary_full_art';
  select coalesce(array_agg(card),'{}'::text[]) into v_standard_available from unnest(v_available) card
    where card not like '%_millennium' and card not like '%_signature' and card not like '%_legendary_full_art'
      and card not like '%_rival' and card not like '%_platinum' and card not like '%_patch' and card not like '%_full_art';
  ...
  elsif cardinality(v_patch_available)>0 and v_roll<0.40 then
    v_card:=v_patch_available[1+floor(random()*cardinality(v_patch_available))::integer];
  elsif cardinality(v_full_art_available)>0 and v_roll<0.62 then
  ...
  elsif cardinality(v_full_art_available)>0 then v_card:=v_full_art_available[1+floor(random()*cardinality(v_full_art_available))::integer];
  elsif cardinality(v_patch_available)>0 then v_card:=v_patch_available[1+floor(random()*cardinality(v_patch_available))::integer];
  elsif cardinality(v_platinum_available)>0 then v_card:=v_platinum_available[1+floor(random()*cardinality(v_platinum_available))::integer];
  elsif cardinality(v_rival_available)>0 then v_card:=v_rival_available[1+floor(random()*cardinality(v_rival_available))::integer];
  elsif cardinality(v_legendary_available)>0 then v_card:=v_legendary_available[1+floor(random()*cardinality(v_legendary_available))::integer];
  elsif cardinality(v_signature_available)>0 then v_card:=v_signature_available[1+floor(random()*cardinality(v_signature_available))::integer];
  else v_card:=v_millennium_available[1+floor(random()*cardinality(v_millennium_available))::integer]; end if;
  ...
  if v_username='admin' then v_items:=jsonb_set(v_items,'{quidditch_tcg_pack}','1'::jsonb,true);
  else v_items:=jsonb_set(v_items,'{quidditch_tcg_pack}',to_jsonb(v_quantity-1),true); end if;

  execute format('update public.characters set bank_items=$1, %I=coalesce(%I,0)+$2, %I=coalesce(%I,0)+$3 where user_id=$4 returning %I,%I',
  ...
  update public.quidditch_tcg_collections set cards=v_owned,opened_count=opened_count+1,updated_at=now() where user_id=auth.uid();
  return query select v_card,v_owned,v_items,v_skill_one,5000,v_total_one,v_skill_two,10000,v_total_two,cardinality(v_owned)>=cardinality(v_all_cards);
end;
$function$;
...
language sql security definer set search_path to 'public' as $function$
  select c.username,coalesce(q.cards,'{}'::text[]),cardinality(coalesce(q.cards,'{}'::text[]))::integer,148::integer
  from public.characters c left join public.quidditch_tcg_collections q on q.user_id=c.user_id
...
language sql security definer set search_path to 'public' as $function$
  select c.username,coalesce(q.cards,'{}'::text[]),cardinality(coalesce(q.cards,'{}'::text[]))::integer,148::integer
  from public.characters c left join public.quidditch_tcg_collections q on q.user_id=c.user_id
  where lower(c.username)=lower(trim(p_username)) and (lower(trim(c.username))<>'admin' or c.user_id=auth.uid()) limit 1
$function$;
