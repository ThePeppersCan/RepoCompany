...
const ARCHETYPE_ICONS={Warrior:'⚔',Ranger:'🏹',Rogue:'🗡',Mage:'✧',Alchemist:'⚗',Bard:'♫',Guardian:'🛡',Beastkeeper:'🐾',Investigator:'🔎'};
  ...
  let state=null,phase2={},view='entry',drawer=null,busy=false,sceneFrame=0,heartbeat=0,saveToastTimer=0,lastNarrative='',activeConversation=null,narrativeTimer=0,lastRenderedNarrative='',uiFxQueue=[],fxActive=false,processingState=null,transitionTimer=0;
  ...
  async function loadFullState(opts={}){
    ...
    return mergePhase23(merged,p23);
  }
  function takePhase2Result(res){
    if(res?.base){
      const existing={local_activities:state?.local_activities||[],activity_mastered_count:Number(state?.activity_mastered_count)||0,activity_total_count:Number(state?.activity_total_count)||0};
      state=mergePhase23(mergePhase2(res.base,res.phase2||{}),res.phase23||existing);return res;
    }
    return res;
  }
...
function cleanupTransientUi(){
  ...
  qAll('.va-processing,.va-scene-transition,.va-banner-stack,.va-dialogue-overlay,.va-dice-overlay,.va-activity-result').forEach(x=>x.remove());
  processingState=null; fxActive=false; uiFxQueue.length=0;
}
...
function afterGameRender(){ setActiveNav(); setNarrative(lastNarrative||state?.adventure?.last_summary||state?.location?.description||'', false); hideProcessing(); flushBanners(); }
...
async function doLocalActivity(key){
  ...
  const res=await callRpc('adventure_phase23_do_activity',{p_activity_key:key}); takePhase2Result(res); lastRenderedNarrative=''; lastNarrative=res.message||state.adventure?.last_summary||''; render(); renderStateFeedback(prev,state,'activity'); showActivityResult(res,before); saved();
}
...
function drawSceneLife(ctx,time,scene,daylight,dusk){
  ...
  if(!daylight){ctx.fillStyle='rgba(255,213,124,.6)';if(scene==='canto_town')ctx.fillRect(96,92,3,3); if(scene==='willowmere')ctx.fillRect(206,87,4,4);}
}
  ...
  async function acceptP2Job(id){const prev=snapshotState();showProcessing('CONTRACT','Taking contract','Routes, fieldwork and obligations settle into place.');const res=await callRpc('adventure_phase2_accept_job',{p_job_id:id});takePhase2Result(res);lastRenderedNarrative='';lastNarrative='The clerk marks the contract active and points out the route on a battered local map.';render();drawer='jobs';renderDrawer();renderStateFeedback(prev,state,'job');saved()}
  async function completeP2Job(id){const prev=snapshotState();showProcessing('CONTRACT','Completing contract','Turning good fieldwork into something that actually pays.');const res=await callRpc('adventure_phase2_complete_job',{p_job_id:id});takePhase2Result(res);lastRenderedNarrative='';lastNarrative=state.adventure.last_summary;render();drawer='jobs';renderDrawer();renderStateFeedback(prev,state,'job');saved()}
  async function acceptP2Quest(key){const prev=snapshotState();showProcessing('QUEST','Following a lead','Quiet mysteries rarely stay quiet for long.');const res=await callRpc('adventure_phase2_accept_quest',{p_quest_key:key});takePhase2Result(res);lastRenderedNarrative='';lastNarrative=state.adventure.last_summary;render();renderStateFeedback(prev,state,'quest');saved()}
  async function p2QuestAction(key,action){const prev=snapshotState();showProcessing('QUEST','Resolving scene','Pausing just long enough for the world to answer back.');const res=await callRpc('adventure_phase2_quest_action',{p_quest_key:key,p_action:action});takePhase2Result(res);lastRenderedNarrative='';lastNarrative=res.message||state.adventure.last_summary;render();renderStateFeedback(prev,state,'quest');saved()}
  async function talkNpc(key,topic='hello'){const prev=snapshotState();showProcessing('CONVERSATION','Approaching conversation','Reading posture, tone and what they might not say aloud.');const res=await callRpc('adventure_phase2_talk',{p_npc_key:key,p_topic:topic});takePhase2Result(res);activeConversation={npc:res.npc,topics:res.topics||{},dialogue:res.dialogue||''};lastRenderedNarrative='';lastNarrative=`${res.npc?.name||'Someone'}: “${res.dialogue||''}”`;render();showConversation(activeConversation);renderStateFeedback(prev,state,'social');saved()}
  ...
  async function inspectHerb(key){const prev=snapshotState();showProcessing('HERBALISM','Inspecting plant','Checking veins, scent and whether it should be cut or left alone.');const res=await callRpc('adventure_phase2_inspect_herb',{p_herb_key:key});takePhase2Result(res);lastRenderedNarrative='';lastNarrative=res.message;render();drawer='codex';renderDrawer();renderStateFeedback(prev,state,'herbalism');saved()}
  async function gatherHerb(key,method){const prev=snapshotState();showProcessing('HERBALISM','Harvesting carefully', `Using ${String(method||'the right method').toUpperCase()} on the plant.`);const res=await callRpc('adventure_phase2_gather_herb',{p_herb_key:key,p_method:method});takePhase2Result(res);lastRenderedNarrative='';lastNarrative=res.message;render();drawer='codex';renderDrawer();renderStateFeedback(prev,state,'herbalism');saved()}
  async function brew(key){const prev=snapshotState();showProcessing('BREWING','Brewing recipe','Measuring, grinding and watching the colour turn.');const res=await callRpc('adventure_phase2_brew',{p_recipe_key:key});takePhase2Result(res);lastRenderedNarrative='';lastNarrative=res.message;render();drawer='codex';renderDrawer();renderStateFeedback(prev,state,'brew');saved()}
  async function wildlifeSearch(){const prev=snapshotState();showProcessing('WILDLIFE','Searching quietly','Following tracks, listening for brush movement and keeping still.');const res=await callRpc('adventure_phase2_wildlife_search');takePhase2Result(res);lastRenderedNarrative='';lastNarrative=res.message;render();renderStateFeedback(prev,state,'wildlife');saved()}
  ...
  async function scout(){const prev=snapshotState();showProcessing('EXPLORATION','Scouting ahead','Looking for an opening beyond the familiar road.');const res=await callRpc('adventure_phase2_scout');takePhase2Result(res);lastRenderedNarrative='';lastNarrative=res.message;render();renderStateFeedback(prev,state,'exploration');saved()}
