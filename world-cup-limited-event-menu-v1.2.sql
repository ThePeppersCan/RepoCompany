-- REPO SPORTS WORLD CUP 2026 — LIMITED EVENT MENU V1.2
-- Adds precise per-slot pack opening so the eight on-screen Event Locker slots
-- are the actual authoritative pack inventory. Existing awards/cards are untouched.
...
begin
  ...
  select e.cards into v_cards
  from public.repo_world_cup_event_2026 e
  where e.user_id=v_uid
  for update;

  select candidate into v_card
  ...
  where pack_id=v_pack.pack_id and user_id=v_uid;

  update public.repo_world_cup_event_2026
  set cards=array_append(coalesce(cards,'{}'::text[]),v_card),updated_at=now()
  where user_id=v_uid;

  select * into v_state
  from public.get_my_world_cup_pack_event_state(p_username);

  return query select
    v_card,v_pack.pack_id,v_pack.source,v_pack.fixture_id,v_pack.fixture_label,v_pack.team_a,v_pack.team_b,v_pack.stage,
