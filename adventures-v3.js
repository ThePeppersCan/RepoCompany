...
const ARCHETYPE_ICONS={Warrior:'⚔',Ranger:'🏹',Rogue:'🗡',Mage:'✧',Alchemist:'⚗',Bard:'♫',Guardian:'🛡',Beastkeeper:'🐾',Investigator:'🔎'};
  ...
  let state=null,phase2={},phase3={},view='entry',drawer=null,busy=false,sceneFrame=0,heartbeat=0,saveToastTimer=0,lastNarrative='',activeConversation=null,lastNpcKey=null,narrativeTimer=0,lastRenderedNarrative='',uiFxQueue=[],fxActive=false,processingState=null,transitionTimer=0,placeholderTimer=0,placeholderIndex=0,dynamicSuggestions=[],commandHistory=[],commandIndex=-1,pendingAmbiguityRaw='',lastPhase3Debug=null;
  ...
  async function loadFullState(opts={}){
    ...
    if(!base?.exists){phase2={};phase3={};return base}
    ...
    return mergePhase3(merged23,p3);
  }
  function takePhase2Result(res){
    if(res?.base){
      ...
      state=mergePhase3(mergePhase23(mergePhase2(res.base,res.phase2||{}),res.phase23||existing23),res.phase3||existing3);return res;
    }
    return res;
  }
...
function renderAdmin(){if(!state.admin)return '<p>Admin only.</p>';const a=state.adventure;return `<div class="va-admin-note">SERVER-VALIDATED TEST PANEL · affects only your Adventures save. Other site systems are untouched.</div><div class="va-admin"><div class="va-admin-row"><label>Adventure XP<input id="vaAdminXp" type="number" min="0" max="13034431" value="${Number(a.xp)||0}"></label><label>Gold<input id="vaAdminGold" type="number" min="0" max="1000000000" value="${Number(a.gold)||0}"></label></div><div class="va-admin-row"><label>World day<input id="vaAdminDay" type="number" min="1" max="9999" value="${Number(a.world_day)||1}"></label><label>Minute of day<input id="vaAdminMinute" type="number" min="0" max="1439" value="${Number(a.world_minute)||480}"></label></div><div class="va-admin-row"><label>Location<select id="vaAdminLocation">${safeArray(state.map_locations).filter(x=>x.discovered).map(x=>`<option value="${esc(x.id)}" ${x.id===a.location_id?'selected':''}>${esc(x.real_name||x.name)}</option>`).join('')}</select></label><label>Weather<select id="vaAdminWeather">${['clear','cloudy','rain','mist','storm','snow','heat'].map(x=>`<option ${x===a.weather?'selected':''}>${x}</option>`).join('')}</select></label></div><button class="va-primary" data-va="admin-apply" type="button">APPLY DEBUG STATE</button></div>`}
  ...
  async function travel(id){const dest=safeArray(state?.map_locations).find(x=>x.id===id);const prev=snapshotState();showSceneTransition('TRAVEL', dest?.real_name||dest?.name||'On the road');showProcessing('TRAVEL', `Heading to ${dest?.real_name||dest?.name||'the next stop'}`, 'Packing satchel, checking weather and taking the road.');const res=await callRpc('adventure_phase2_travel',{p_location_id:id});takePhase2Result(res);await refreshPhase3();view='game';lastRenderedNarrative='';lastNarrative=state.adventure.last_summary;dynamicSuggestions=[];render();renderStateFeedback(prev,state,'travel');hideSceneTransition();saved()}
  ...
  async function scout(){const prev=snapshotState();showProcessing('EXPLORATION','Scouting ahead','Looking for an opening beyond the familiar road.');const res=await callRpc('adventure_phase2_scout');takePhase2Result(res);await refreshPhase3();lastRenderedNarrative='';lastNarrative=res.message;render();renderStateFeedback(prev,state,'exploration');saved()}
